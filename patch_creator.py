...
class CustomPatchset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        image = cv2.imread(image_path, cv2.IMREAD_COLOR)
        image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
        return image, torch.tensor(label), torch.tensor(self.X_train.iloc[index]), torch.tensor(index)
...
def run():
    ...
    for batch_idx, data in enumerate(tqdm(train_loader, total=len(train_loader))):
        images, targets = data
        patch_dataset = CustomPatchset(root_dir, X_train, y_train, train_transforms)
        patch_loader = DataLoader(patch_dataset, batch_size = PATCH_BATCH_SIZE, shuffle=True, num_workers = num_workers)
        for batch_idx, data in enumerate(tqdm(train_loader, total=num_patches*num_patches)):
            images, targets, patch_id, patch_ind = data
